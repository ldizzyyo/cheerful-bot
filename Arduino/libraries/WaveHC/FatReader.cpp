 ...
 *  exist, or an I/O error occurred.  
 */  
uint8_t FatReader::open(FatReader &dir, const char *name) {
  dir_t entry;
  char dname[13];
...
///Serial.print("read count="); Serial.println(count, DEC);
  uint8_t *dst = (uint8_t *)buf;
  uint16_t nr = 0;
  int16_t n = 0;
  while (nr < count && (n = readBlockData(dst, count - nr)) > 0) {
///Serial.print("read n="); Serial.println(n, DEC);
    if (!seekCur(n)) return -1;
    dst += n;
    nr += n;
///Serial.print("read nr="); Serial.println(nr, DEC); Serial.print("read dst="); Serial.println((long int) dst, DEC);
  }
  return n < 0 ? -1 : nr;
}
//------------------------------------------------------------------------------
...
///Serial.print("readBlockData, count="); Serial.println(count, DEC);
  uint32_t block;
...
///Serial.print("dir "); Serial.print("n="); Serial.println(n, DEC);
